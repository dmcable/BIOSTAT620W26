...
author: "George G. Vega Yon, Kelly Street, and Dylan Cable"
format:
  revealjs:
    theme: default
    fig-width: 7
...
Download the program [here](img/sars-cov2.R).
...
- Great Lakes HPC Cluster (ARC). UMich users can request hundreds
  of cores (literally).
...
Imagine we have the following R script (download [here](img/dummy.R)):
...
```{bash since-born, eval=FALSE, echo = TRUE}
...
In the case of running jobs on a cluster or something similar, we usually need to have a bash script. In our case, we have a file named `since_born_bash.sh` that calls `Rscript` (download [here](img/since_born_bash.sh))
...
## RcppArmadillo and OpenMP {.smaller}

*   Friendlier than [**RcppParallel**](http://rcppcore.github.io/RcppParallel/)...
    at least for 'I-use-Rcpp-but-don't-actually-know-much-about-C++' users.
...
## RcppArmadillo and OpenMP workflow {.smaller}
...
2.  Tell the compiler that you'll be running a block in parallel with openmp
    ...
    ```cpp
    #pragma omp [directives] [!q](location)
    {
...
<figure>
<img src="https://upload.wikimedia.org/wikipedia/commons/a/ae/SISD.svg" style="width:23%;">
<img src="https://upload.wikimedia.org/wikipedia/commons/9/97/MISD.svg" style="width:23%;">
<img src="https://upload.wikimedia.org/wikipedia/commons/2/21/SIMD.svg" style="width:23%;">
<img src="https://upload.wikimedia.org/wikipedia/commons/c/c6/MIMD.svg" style="width:23%;">
