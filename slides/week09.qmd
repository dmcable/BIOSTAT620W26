---
title: "SQL"
subtitle: "BIOSTAT620 - Week 9"
author: "Emil Hvitfeldt, George G. Vega Yon, Kelly Street, and Dylan Cable"
format:
  revealjs:
    theme: default
    fig-width: 7
...
- **Table** - A set of rows
- **Result table** - Another name for a non-persistent table, generally the result of a SQL query
...
When using SQL we specify the input and output we desire, and then this is sent to your database engine known as the **optimizer**
...
Consider the following SQL operations: **SELECT**, **FROM**, **WHERE**, **GROUP BY**, **HAVING**, **ORDER BY**. 

Complete the following analogy (choosing X, Y, and Z from the operations above): X for Y is like Z for rows.
...
We first start by loading the datasets from https://github.com/dmcable/BIOSTAT620W26

```{r, echo=TRUE}
# Download
chs_regional   <- read.csv("https://github.com/dmcable/BIOSTAT620W26/raw/main/data/chs/chs_regional.csv")
chs_individual <- read.csv("https://github.com/dmcable/BIOSTAT620W26/raw/main/data/chs/chs_individual.csv")
