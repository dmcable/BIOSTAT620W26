...
author: "Dylan Cable, George G. Vega Yon, and Kelly Street"
format:
  revealjs:
    theme: default
    fig-width: 7
...
library(stringr)
txt <- c("Hanna Perez [name]", "The 年 year was 1999", "HaHa, @abc said that", "GoGo wolverines #2026!")
regex <- c(
  ".{5}",
  "n{2}",
  "[0-9]",
...
mts_url <- "https://github.com/dmcable/BIOSTAT620W26/raw/main/data/mtsamples/mtsamples.csv"
...
using the words *he, his, him, they, them, she, her*:

```{r, echo=TRUE}
mtsamples$pronoun <- str_extract(
  string  = tolower(mtsamples$transcription),
  pattern = "he|his|him|they|them|she|her"
)
```
...
`(?<=\W|^)(he|his|him|they|them|she|her)(?=\W|$)`
...
mtsamples$pronoun <- str_extract(
  string  = tolower(mtsamples$transcription), 
  pattern = "(?<=\\W|^)(he|his|him|they|them|she|her)(?=\\W|$)"
  )
...
table <- xml_find_first(covid, xpath = '//div[@class="sticky-table-scroll"]/table')
...
```{r example-gene-ontology, cache=FALSE, echo=TRUE}
library(httr)
go_query <- GET(
...
```{r example-gene-ontology-cont, cache=FALSE, echo=TRUE}
go_query
```
...
```{r health-advice, cache=FALSE, echo=TRUE}
health_advises <- GET(
  url = "https://odphp.health.gov", 
  path  = "myhealthfinder/api/v4/myhealthfinder.json",
  query = list(
    lang = "en",
    age  = "32",
    sex  = "male",
    tobaccoUse = "no" # Note: v4 often uses "yes"/"no" instead of 0/1
  ),
  config = c(
...
> ans$request
# <request>
# GET https://dashboard.healthit.gov/api/open-api.php?source=AHA_2008-2015.csv&region=Michigan&period=2015
# Output: write_memory
# Options:
# * useragent: libcurl/7.58.0 r-curl/4.3 httr/1.4.1
# * connecttimeout: 60
