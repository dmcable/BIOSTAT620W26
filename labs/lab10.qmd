...
Due date: Tuesday, March 31st at 8:30am EST.
...
```{r}
# format the date
cv_states$date <- ### FINISH THE CODE HERE

# format the state and state abbreviation (abb) variables
cv_states$state <- factor(### FINISH THE CODE HERE)
cv_states$abb <- factor(### FINISH THE CODE HERE)
...
- Correct implausible values: Set negative values for `new_cases` or `new_deaths` to 0
...
cv_states$per100k <- as.numeric(format(round(cv_states$________/(cv_states$________/100000),1),nsmall=1))
cv_states$newper100k <- as.numeric(format(round(cv_states$________/(cv_states$________/100000),1),nsmall=1))
cv_states$deathsper100k <- as.numeric(format(round(cv_states$________/(cv_states$________/100000),1),nsmall=1))
cv_states$newdeathsper100k <- as.numeric(format(round(cv_states$________/(cv_states$________/100000),1),nsmall=1))

# add a naive_CFR variable = deaths / cases
cv_states <- cv_states |> mutate(naive_CFR = round((________),2))
...
cv_states_today_filter <- cv_states_today |> filter(state!="_________")

# pop_density vs. cases after filtering
cv_states_today_filter |> 
  plot_ly(x = ~pop_density, y = ~cases, 
          type = _______, mode = _______, color = ~_______,
          size = ~population, sizes = c(5, 70), marker = list(sizemode='diameter', opacity=0.5))
...
cv_states_today_filter |> 
  plot_ly(x = ~pop_density, y = ~deathsper100k,
          type = _______, mode = _______, color = ~_______,
          size = ~population, sizes = c(5, 70), marker = list(_______))

# Adding hoverinfo
cv_states_today_filter |> 
  plot_ly(x = ~pop_density, y = ~deathsper100k,
          type = _______, mode = _______, color = ~_______,
          size = _______, sizes = _______, marker = list(_______),
          hoverinfo = 'text',
          text = ~paste( paste(_______, ":", sep=""), paste(" Cases per 100k: ", _______, sep="") , 
...
```{r}
### FINISH CODE HERE
p <- ggplot(cv_states_today_filter, aes(x=_______, y=_______, size=population)) + geom_point() + _______
ggplotly(p)
```


### 8. Multiple line charts
...
- Create one more line chart, for Florida only, which shows `new_cases` and `new_deaths` together in one plot. Hint: look for an `add_*()`
  ...
  - HINT: select mult factor to be approximately 1 / CFR (and also a round number) to get cases and deaths on the same scale.
...
plot_ly(cv_states, _______)

### FINISH CODE HERE
MULT_FACTOR <- _____ ### HINT: select mult factor to be approximately 1 / CFR (and also a round number) to get cases and deaths on the same scale.
# Line chart for Florida showing new_cases and new_deaths together (two lines)
cv_states |> filter(state=="Florida") |> 
  plot_ly(x = ~date, y = ~new_cases/MULT_FACTOR, type = "scatter", mode = "lines") |> 
...
cv_states_mat2 <- as.data.frame(pivot_wider(cv_states_mat, names_from = ________, values_from = ________))
...
# Create the map
fig <- _______(cv_per100, locationmode = 'USA-states') |> 
  add_trace(
...
# Create hover text to include state name, new cases per 100k, cases, and deaths
cv_per100$hover <- with(cv_per100, _______________)

# Set up mapping details
set_map_details <- list(
  scope = 'usa',
  projection = list(type = _______),
  showlakes = TRUE,
  lakecolor = toRGB('white')
)

# Create the map
fig <- plot_geo(cv_per100, locationmode = _______) |> 
  add_trace(
    z = ~newper100k, text = ~hover, locations = ~state,
    color = ~newper100k, colors = 'Purples'
  )
